...
> Thunderbird 邮件待办识别与日历导入插件。主界面以“邮件”页右侧第四栏常驻显示，支持 LLM 识别（云端/本地）与本地规则识别，提供可视化审核后再导入日历。
...
### 3) 确认按钮可见
安装后你应该在工具栏或邮件阅读相关入口看到 `Todo Sidebar` 按钮；在 Thunderbird 邮件页中，插件主界面会作为右侧第四栏显示。
...
第四栏说明：
- 点击工具栏按钮会显示或聚焦右侧第四栏，而不是弹出临时 popup。
- 可拖动第四栏左侧分隔条调整宽度，插件会记住上次宽度。

## 30 秒快速开始
1. 打开 Thunderbird 邮件页，确认右侧第四栏已显示。
2. 点击 `扫描未读`。
3. 在账号弹窗里勾选要扫描的邮箱，点 `开始扫描`。
4. 查看状态栏（底部）进度：例如“状态：上传 x/y 封邮件”。
5. 在中间分组区单击事件：
   - 打开对应邮件
   - 展开标题/时间/地点详情
6. 对事件做决策（确认/拒绝/标记已读/转为待办）。
7. 点击 `导入日历`，选择目标日历并确认。
8. 到 `已添加日历` 分组核验导入记录。
